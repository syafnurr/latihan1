        ...
        <div class="grid lg:grid-cols-2 lg:h-full">
            <div class="flex justify-center items-center py-6 px-4 lg:py-6 sm:px-0">
                <div class="w-full space-y-4 md:space-y-6 max-w-lg xl:max-w-lg p-6">
                    ...
                    <x-forms.form-open class="space-y-4 md:space-y-6" :action="route('admin.login.post')" method="POST" />
                    <x-forms.input type="email" name="email" icon="envelope" :label="trans('common.email_address')" :placeholder="trans('common.your_email')"
                        :required="true" />
                    <x-forms.input type="password" name="password" icon="key" :label="trans('common.password')" :placeholder="trans('common.password')"
                        :required="true" />
                    <div class="flex items-center justify-between">
                        <div class="flex items-start">
                            <x-forms.checkbox name="remember" :label="trans('common.remember_me')" />
                        </div>
                        <a href="{{ route('admin.forgot_password') }}" class="text-sm font-medium text-primary-600 hover:underline dark:text-primary-500">{{ trans('common.forgot_password') }}</a>
                    </div>
