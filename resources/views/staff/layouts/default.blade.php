                ...
                <div class="flex flex-wrap justify-between items-center px-3 md:px-3 py-2.5">
                    ...
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('staff.qr.scanner') }}" class="md:hidden flex text-sm rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                            <x-ui.icon icon="qr-code" class="m-1 w-7 h-7 text-gray-900 dark:text-gray-300"/>
                        </a>
                        <span class="md:hidden flex mr-3 ml-3 w-px h-5 bg-gray-200 dark:bg-gray-600"></span>
                        <div class="hidden md:flex items-center">
                            <button type="button" class="flex text-sm rounded-full md:mr-3 mr-2 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom">
                                <span class="sr-only">{{ trans('common.open') }}</span>
                                @if(auth('staff')->user()->avatar)
                                  <img class="w-8 h-8 rounded-full" src="{{ auth('staff')->user()->avatar }}">
                                @else
                            ...
                            <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-2xl dark:bg-gray-700 dark:divide-gray-600" id="user-dropdown">
                              <div class="px-4 py-3">
                                <span class="block text-sm text-gray-900 dark:text-white">{{ auth('staff')->user()->name }}</span>
                                <span class="block text-sm font-medium text-gray-500 truncate dark:text-gray-400">{{ auth('staff')->user()->email }}</span>
                              </div>

                              <ul class="py-1 font-light text-gray-500 dark:text-gray-400" aria-labelledby="user-menu-button">
                                 <li>
                                    <a href="{{ route('staff.index') }}" class="block py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400 dark:hover:text-white @if ($routeName == 'staff.index') text-black dark:text-white @endif">{{ trans('common.dashboard') }}</a>
                                 </li>
                                 <li>
                                    <a href="{{ route('staff.data.list', ['name' => 'account']) }}" class="block py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400 dark:hover:text-white @if ($routeDataDefinition == 'account') text-black dark:text-white @endif">{{ trans('common.account_settings') }}</a>
                                </li>
                              </ul>
                              <ul class="py-1 font-light text-gray-500 dark:text-gray-400" aria-labelledby="dropdown">
                                  <li>
                                      <a href="{{ route('staff.logout') }}" class="block py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ trans('common.logout') }}</a>
                                  </li>
                              </ul>
                            </div>
                        </div>
                    ...
                    <div class="grid py-4 px-4 mx-auto max-w-screen-2xl lg:grid-cols-2 md:px-6">
                        ...
                        <div class="flex items-center">
                            <ul class="flex flex-row items-center mt-0 space-x-8 text-sm font-medium">
                                <li>
                                    <a href="{{ route('staff.index') }}" class="hover:text-black dark:hover:text-white @if ($routeName == 'staff.index') text-gray-900 dark:text-white @else text-gray-700 dark:text-gray-400 @endif">{{ trans('common.dashboard') }}</a>
                                </li>
                                <li>
                                    <a href="{{ route('staff.qr.scanner') }}" class="hover:text-black dark:hover:text-white @if ($routeName == 'staff.qr.scanner') text-gray-900 dark:text-white @else text-gray-700 dark:text-gray-400 @endif">{{ trans('common.scan_qr') }}</a>
                                </li>
                                <li>
                                    <a href="{{ route('staff.data.list', ['name' => 'members']) }}" class="hover:text-black dark:hover:text-white @if ($routeDataDefinition == 'members') text-gray-900 dark:text-white @else text-gray-700 dark:text-gray-400 @endif">{{ trans('common.members') }}</a>
                                </li>
                            </ul>
                        </div>
                    </div>
            ...
            <div id="drawer-navigation" class="fixed z-40 h-screen p-4 overflow-y-auto bg-white w-80 dark:bg-gray-800 transition-transform left-0 top-0 -translate-x-full" tabindex="-1">
                ...
                <div class="py-4 overflow-y-auto mb-24">
                    ...
                    <ul class="space-y-2 site-drawer-nav">
                        <li @if ($routeName == 'staff.index') class="active" @endif>
                            <a href="{{ route('staff.index') }}"><x-ui.icon icon="home" class="w-6 h-6" /><span>{{ trans('common.dashboard') }}</span></a>
                        </li>
                        <li><hr class="h-px my-5 bg-gray-200 border-0 dark:bg-gray-700"></li>
                        <li @if ($routeName == 'staff.qr.scanner') class="active" @endif>
                            <a href="{{ route('staff.qr.scanner') }}"><x-ui.icon icon="qr-code" class="w-6 h-6" /><span>{{ trans('common.scan_qr') }}</span></a>
                        </li>
                        <li @if ($routeDataDefinition == 'members') class="active" @endif>
                            <a href="{{ route('staff.data.list', ['name' => 'members']) }}"><x-ui.icon icon="user-group" class="w-6 h-6" /><span>{{ trans('common.members') }}</span></a>
                        </li>
                        <li><hr class="h-px mt-5 bg-gray-200 border-0 dark:bg-gray-700"></li>
                        <li><h5 class="inline-flex items-center ml-2 my-4 text-sm font-medium text-gray-400 dark:text-gray-400">{{ auth('staff')->user()->name }}</h5></li>
                        <li @if ($routeDataDefinition == 'account') class="active" @endif>
                            <a href="{{ route('staff.data.list', ['name' => 'account']) }}"><x-ui.icon icon="user-circle" class="w-6 h-6" /><span>{{ trans('common.account_settings') }}</span></a>
                        </li>
                        <li><a href="{{ route('staff.logout') }}"><x-ui.icon icon="power" class="w-6 h-6" /><span>{{ trans('common.logout') }}</span></a></li>
                    </ul>
