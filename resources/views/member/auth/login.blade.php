        ...
        <div class="grid lg:grid-cols-2 lg:h-full">
            <div class="flex justify-center items-center py-6 px-4 lg:py-6 sm:px-0">
                <div class="w-full space-y-4 md:space-y-6 max-w-lg xl:max-w-lg p-6">
                    ...
                    <div class="flex p-4 mb-4 text-sm text-blue-800 border-2 border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800 cursor-pointer" role="alert" onclick="document.getElementById('email').value='member@example.com';document.getElementById('password').value='welcome123';">
                        <x-ui.icon icon="info" class="flex-shrink-0 w-5 h-5" />
                        <div class="ml-3 text-sm font-medium">
                            To access the demo, use the following login credentials: <span class="font-extrabold">member@example.com / welcome123</span> (click to autofill).
                        </div>
                      </div>
                    @endif
                    ...
                    <x-forms.messages />
                    <x-forms.form-open class="space-y-4 md:space-y-6" :action="route('member.login.post')" method="POST" />
                    <x-forms.input
                        type="email"
                        name="email"
                        icon="envelope"
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('member.forgot_password') }}" class="text-sm font-medium text-primary-600 hover:underline dark:text-primary-500">{{ trans('common.forgot_password') }}</a>
                    </div>
            ...
            <div class="flex justify-center items-center py-6 px-4 bg-primary-600 lg:py-0 sm:px-0">
                <div class="max-w-md xl:max-w-xl p-6">
                    <h1 class="mb-4 text-3xl font-extrabold tracking-tight leading-none text-white xl:text-5xl">
                        {!! trans('common.login_block_title') !!}</h1>
